const onno = {
  // Array
  randomOfArray: (array = [false, true]) => {
    return array[Math.floor(Math.random() * array.length)]
  },
  // Object, Key, Descending or Ascending
  arrayOfObjectSort: (object, key, sort) => {
  ...
  // Maximum, Minimum
  randomNumber: (max = 1, min = 0) => {
    return Math.floor(Math.random() * (max - min + 1) + min)
  },
  // Parent, InnerHTML, Class, Id
  html: (parent = "div", innerH = "", classs = "", iddd = "") => {
  ...
  // IF Seconds, new Date()
  timeDate: (ifSec = true, current = new Date()) => {
    const config = {
      hour: "2-digit",
      day: "2-digit",
      hour: "2-digit",
  ...
  typeWriter: (element, txt = [], wS = 75, cS = 50, wD = 500, cD = 1000, count = false, clean = true, fn = () => {}) => {
    let txtIndex = 0
    let i = 0
    let counter = 1
    const clear = () => {
  ...
  cookie: function (key, value, day = 0, hour = 0, minute = 0, second = 0) {
